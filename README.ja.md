...
# Tailscaleノード（100.64.0.0/10、*.ts.net）への接続は通常の公開鍵認証を使用
...
| 優先度 | 方式        | 説明                                                                                    |
|--------|-------------|-----------------------------------------------------------------------------------------|
| 1      | Tailscale   | `tailscale whois` CLI優先。未インストール時はCGNATレンジ/MagicDNS (*.ts.net) で判定     |
...
**プライマリ**: `tailscale whois --json` でローカルTailscaleデーモンに問い合わせ、権威あるピア情報を取得します。

**フォールバック**（CLIが利用できない場合）: SSH接続先の解決済みホスト名を以下の基準で判定します:

- Tailscale CGNATレンジ (`100.64.0.0/10`) のIPアドレス
- MagicDNSホスト名 (`*.ts.net`)
- DNS解決後のIPがCGNATレンジに含まれる場合
