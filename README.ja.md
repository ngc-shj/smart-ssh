...
```bash
# Tailscaleホストをホームネットワークとして扱う（true/false）
# ローカルTailscaleデーモンでのみ検証。jqとtailscale CLIが必要
TAILSCALE_AS_HOME=true

# PATHにtailscale CLIが無い場合の探索先（macOS App Store版）
# 信頼するにはrootが所有している必要があります
...
| 優先度 | 方式        | 説明                                                                                    |
|--------|-------------|-----------------------------------------------------------------------------------------|
| 1      | Tailscale   | `tailscale whois` CLIによる判定のみ（フォールバックなし。下記参照）                     |
| 2      | Gateway MAC | ARP経由のルーターMACアドレス（権限不要）                                                |
| 3      | IPアドレス  | CIDR範囲マッチング（フォールバック）                                                    |

### Tailscale検出

`TAILSCALE_AS_HOME=true`（デフォルト）の場合、Tailscaleノードへの接続は自動的にホームネットワークとして扱われます。

判定はローカルTailscaleデーモンにのみ問い合わせます。SSH接続先の `HostName` を
デーモン自身のピアリスト（`tailscale status --json`）と照合してtailnetアドレスを
求め、そのアドレスを `tailscale whois --json` で確認します（whoisは名前ではなく
アドレスを受け取ります）。`HostName` はtailnetアドレス、MagicDNS FQDN（末尾ドット
の有無を問わず）のいずれでも構いません。ノード名の短縮形は受け付けません
（LANのマシンに `HostName nas` と書いている場合、たまたま `nas` という名前の無関係な
tailnetピアと一致してしまうためです）。

名前解決に `tailscale ip <name>` は意図的に使いません。デーモンが知らない名前に
対して、このコマンドはシステムDNSで引き直し、返ってきたアドレスの持ち主のピアを
報告します。信頼できないネットワークではリゾルバは攻撃者のものなので、この判定の
対象となるアドレスを攻撃者に選ばせることになります。ピアリストとの照合であれば
DNSが経路に入りません。

CLIが見つからない、デーモンが動作していない、`jq` が利用できない場合、smart-sshは
外部接続として扱い、セキュリティキーを要求します。**ヒューリスティックによる
フォールバックは意図的に持ちません。**

以前のバージョンは、デーモンに問い合わせられない場合に宛先から推測していました
（CGNATレンジ `100.64.0.0/10` のアドレス、`*.ts.net` 名、DNS解決結果が同レンジに
入ること）。しかしこれらはいずれも証拠になりません。`100.64.0.0/10` は任意のDHCP
サーバーが配布しうる共有CGNAT空間であり、`*.ts.net` は経路の実在ではなく自分の
設定に書いたラベルにすぎず、信頼できないネットワークではリゾルバもその運用者の
ものです。どれもセキュリティキーが守ろうとしている当のネットワークが作り出せる
ため、これらを根拠にすると、まさに使ってはいけない場所でオンディスク鍵を使うこと
になります。

ノードが確認できた場合、smart-sshは**デーモンが保証したアドレス**へ接続します
（`-o HostName=<tailnetアドレス> -o HostKeyAlias=<名前>`）。ssh に名前を引き直さ
せません。この判定が示すのは「その名前のピアがtailnet上に存在すること」であって
「その名前が今もそのピアに解決されること」ではなく、TailscaleがDNSを管理していない
状況（`--accept-dns=false`、MagicDNS無効、`resolv.conf` を掌握できないプラット
フォーム）では、その2回目の解決が信頼できないネットワークのリゾルバに向かうため
です。`HostKeyAlias` により `known_hosts` の項目は入力した名前のまま保たれるので、
ホスト鍵の確認を追加で求められることはありません。自分で `HostKeyAlias` を設定
している場合はそちらを尊重します。
...
経由します。指定した設定は `ProxyCommand`、`HostKeyAlias`、
`UserKnownHostsFile`、各種フォワードを含めてそのまま一時設定に取り込み、認証に
関わるディレクティブだけを差し替えます。`-F` はその後、ssh のコマンドラインから
除去します。ssh は最後の `-F` を採用するため、そのまま残すと一時設定が
置き換えられ、認証の制約が失われるからです。`-F` 以外のオプションはそのまま渡します。固定オプションは利用者自身のオプションより前に置きます。sshは
同一オプションの最初の値を採用するため、後ろに足しても無視されるからです。

CLIは `PATH` に加えて、macOS App Store版がシンボリックリンクを作らずに配置する
`/Applications/Tailscale.app/Contents/MacOS/Tailscale` も探索します。後者は
tailscale CLIが `PATH` に無い場合にのみ参照し、かつrootが所有している場合だけ実行
します（macOSの `/Applications` はグループ書き込み可能なため、非特権プロセスが
任意のホストを保証する偽CLIを置けてしまいます）。配置が異なる場合は
`TAILSCALE_CLI_BUNDLE_PATH`（環境変数または設定ファイル）で指定してください。

デーモンが利用できない状況でホーム扱いにしたい場合は、ゲートウェイMAC検出または
`HOME_NETWORK` を使ってください。いずれも以下で説明しており、接続先の名前に依存
しません。
...
- OIDCプロビジョナーを設定した [step-ca](https://smallstep.com/docs/step-ca/)
- `OIDC_CA_URL` で指定したURLにクライアントからCAに到達できること

#### クライアントシークレットの扱い

**パブリッククライアント（シークレットなし）を強く推奨します。** シークレットなしでDevice Flowグラントを許可するようOIDCプロバイダーを設定してください。

シークレットが避けられない場合:

- 設定ファイルへの `OIDC_CLIENT_SECRET` の保存は **安全ではありません**（デフォルトでは誰でも読める）
- 代わりにアクセス権を制限した環境変数として保存してください:

```bash
# アクセス権を制限したシェルプロファイルに設定
chmod 600 ~/.bashrc  # または ~/.zshrc
...
| 変数                 | 説明                                      | デフォルト          |
|----------------------|-------------------------------------------|---------------------|
| `OIDC_CERT_LIFETIME` | 有効期間（秒）最大86400秒（24時間）       | `3600`              |
| `OIDC_CERT_DIR`      | 発行済み証明書のキャッシュディレクトリ    | `~/.ssh/oidc-certs` |
...
証明書の発行はCAサイドでログに記録されます。step-caのログでOIDC発行証明書の監査記録を確認してください。

#### クライアントの要件

- `jq` — JSONパース
- `curl` — HTTPリクエスト
- Device Authorization Flow (RFC 8628) をサポートするOIDCプロバイダー（Google、Keycloak、Oktaなど）
...
smart-sshのクライアントサイドネットワーク検出はすべて **UX上の利便性機能** であり、セキュリティ境界ではありません。実際のセキュリティ制御はサーバーサイドの強制です。

### 保護されるもの

- **サーバーサイドの送信元IP強制**: `sshd_config` の `Match Address` により、クライアントの送信元IPに基づいて許可される鍵アルゴリズムと認証方式が制限されます。クライアントはこれを回避できません。

### 保護されないもの

クライアントサイドの検出は、ネットワークまたはクライアントマシンを制御する攻撃者を防ぐことはできません。

| 検出方式            | 制限事項                                                                                    |
|---------------------|---------------------------------------------------------------------------------------------|
| `tailscale whois`   | ローカルTailscaleデーモンへの信頼に依存。ローカル権限分離を超える保証は提供しない           |
| Gateway MAC         | 敵対的環境ではARPスプーフィングにより簡単に偽装可能                                         |
| IP/CIDR             | 任意のネットワークが同じアドレスレンジを使用できる。自宅ネットワークを一意に識別しない      |

### 設計の意図

smart-sshはクライアントサイド検出を、検出されたコンテキストに対して最も便利な認証方式を選択するためにのみ使用します。検出が欺かれた場合でも、サーバーは強制ポリシーに合致しない認証試行を拒否します（例: 外部IPから接続する場合、`Match all` ブロックが `touch-required` セキュリティキーを要求するため、通常の鍵はサーバーに拒否されます）。
