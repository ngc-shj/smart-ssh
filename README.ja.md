...
| 優先度 | 方式        | 説明                                                            |
|--------|-------------|-----------------------------------------------------------------|
| 1      | Tailscale   | 接続先がTailscale CGNATレンジまたはMagicDNS (*.ts.net) のホスト |
| 2      | Gateway MAC | ARP経由のルーターMACアドレス（権限不要）                        |
| 3      | IPアドレス  | CIDR範囲マッチング（フォールバック）                            |

### Tailscale検出

`TAILSCALE_AS_HOME=true`（デフォルト）の場合、Tailscaleノードへの接続は自動的にホームネットワークとして扱われます。SSH接続先の解決済みホスト名を以下の基準で判定します:
